...
CREATE TABLE "commande" (
  "id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "id_catalogue_consommable" integer NOT NULL,
  --
...
BEGIN
  IF NEW.date_reception <= CURRENT_DATE AND NEW.reception = true THEN
    UPDATE consommable
    SET stock = stock + (NEW.nombre_lot * (SELECT quantite_par_unite FROM consommable WHERE id = (SELECT id_consommable FROM catalogue_consommable WHERE id = NEW.id_catalogue_consommable)))
    WHERE id = (SELECT id_consommable FROM catalogue_consommable WHERE id = NEW.id_catalogue_consommable);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE "commande" ADD FOREIGN KEY ("id_catalogue_consommable") REFERENCES "catalogue_consommable" ("id");
