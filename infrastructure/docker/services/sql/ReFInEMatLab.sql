...
CREATE TABLE "appareil" (
  ...
  "etat_fonctionnement" varchar(50),
...
CREATE TABLE "appartient" (
  ...
  PRIMARY KEY ("id_ville", "id_code_postal")
);
...
-- Add a check constraint to ensure that id_entreprise is NULL when id_externe_interne.type is internal and not NULL when id_externe_interne.type is external
CREATE OR REPLACE FUNCTION check_id_entreprise() RETURNS TRIGGER AS $$
BEGIN
  IF NEW.id_externe_interne = (SELECT id FROM externe_interne WHERE type = 'interne') AND NEW.id_entreprise IS NOT NULL THEN
    RAISE EXCEPTION 'id_entreprise must be NULL for internal id_externe_interne';
  ELSIF NEW.id_externe_interne = (SELECT id FROM externe_interne WHERE type = 'externe') AND NEW.id_entreprise IS NULL THEN
    RAISE EXCEPTION 'id_entreprise must not be NULL for external id_externe_interne';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create the trigger
CREATE TRIGGER check_id_entreprise_trigger
BEFORE INSERT OR UPDATE ON personne
FOR EACH ROW EXECUTE PROCEDURE check_id_entreprise();
...
CREATE UNIQUE INDEX "idx_code_postal_id_region_code_postal" ON "code_postal" ("id_region", "code_postal");

CREATE UNIQUE INDEX "idx_region_id_pays_nom" ON "region" ("id_pays", "nom");
...
ALTER TABLE "appartient" ADD FOREIGN KEY ("id_ville") REFERENCES "ville" ("id");

ALTER TABLE "appartient" ADD FOREIGN KEY ("id_code_postal") REFERENCES "code_postal" ("id");

ALTER TABLE "code_postal" ADD FOREIGN KEY ("id_region") REFERENCES "region" ("id");

ALTER TABLE "region" ADD FOREIGN KEY ("id_pays") REFERENCES "pays" ("id");

ALTER TABLE "entreprise" ADD FOREIGN KEY ("id_ville") REFERENCES "ville" ("id");
